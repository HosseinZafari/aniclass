module.exports = class TeacherModel {
    ...
    createTeacher  = async ({ national_code , name , family , email , password , qrcode}) => {
        try {
            const isAcceptForRegister = await this.db.pool.query("SELECT * FROM university_tb WHERE qrcode=$1;" , [qrcode]);
            if(isAcceptForRegister.rowCount === 0) {
                return -1;
            }
